...
class Section_sound_lights extends Model
{
    ...
    {
        if($id == ''){
            $slider           = new Section_sound_lights($request->except('_token', 'picture'));
            $slider->save();
        }else{
            $slider = Section_sound_lights::find($id);
        ...
        if($request->hasFile('picture') != false) { //image is not empty coverfile
            ...
            $new_image = Section_sound_lights::find($slider->id);
